...
template <typename Dtype>
Dtype WeightedSigmoidCrossEntropyLossLayer<Dtype>::get_normalizer(
...
void WeightedSigmoidCrossEntropyLossLayer<Dtype>::Backward_cpu(
  ...
  if (propagate_down[0]) {
    ...
    const Dtype* target = bottom[1]->cpu_data();
    const Dtype* diff_l = weights_.cpu_diff();
    ...
    for (int i = 0; i < count; ++i) {
      ...
      if (target[i] == 1) {
          bottom_diff[i] = diff_l[i] * (one - weights[i]) * sigmoid_output_data[i] * weights[i] - 
              weights[i] * weights[i] * total_weight_;
      } else {
...
#ifdef CPU_ONLY
STUB_GPU(WeightedSigmoidCrossEntropyLossLayer);
#endif
