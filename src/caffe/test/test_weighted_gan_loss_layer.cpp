...
class WeightedGanLossLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  WeightedGanLossLayerTest()
        : blob_bottom_data_(new Blob<Dtype>(10, 5, 1, 1)),
          blob_bottom_targets_(new Blob<Dtype>(10, 5, 1, 1)),
          blob_bottom_weights_(new Blob<Dtype>(10, 5, 1 ,1)),
  ...
  virtual ~WeightedGanLossLayerTest() {
  ...
  Dtype WeightedGanLossReference(const int count, const int num,
                                         const Dtype* input,
                                         const Dtype* target,
                                         const Dtype* weights) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      if (target[i] == 1) {
        const Dtype weight = weights[i] / total_weight;
        EXPECT_LE(weight, 1);
        EXPECT_GE(weight, 0);
  ...
  void TestForward() {
    ...
    for (int i = 0; i < 100; ++i) {
      ...
      WeightedGanLossLayer<Dtype> layer(layer_param);
      ...
      const Dtype* blob_bottom_weights = this->blob_bottom_weights_->cpu_data();
      Dtype reference_loss = kLossWeight * WeightedGanLossReference(
          count, num, blob_bottom_data, blob_bottom_targets, blob_bottom_weights);
...
TYPED_TEST_CASE(WeightedGanLossLayerTest, TestDtypesAndDevices);

TYPED_TEST(WeightedGanLossLayerTest, TestWeightedGanLoss) {
  this->TestForward();
}

TYPED_TEST(WeightedGanLossLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  const Dtype kLossWeight = 3.7;
  layer_param.add_loss_weight(kLossWeight);
...
TYPED_TEST(WeightedGanLossLayerTest, TestIgnoreGradient) {
  typedef typename TypeParam::Dtype Dtype;
  FillerParameter data_filler_param;
  data_filler_param.set_std(1);
  GaussianFiller<Dtype> data_filler(data_filler_param);
